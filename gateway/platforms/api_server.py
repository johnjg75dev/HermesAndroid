...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Queued user inputs per session key (busy-mode composer). Messages
        # posted to /v1/runs/{id}/queue while a run is active are chained
        # into a fresh run when the current one completes.
        self._run_input_queues: Dict[str, "deque"] = {}
    ...
    def _create_agent(
        ...
        if os.getenv("HERMES_ANDROID_BOOTSTRAP", "").strip():
            try:
                from hermes_android.config.defaults import should_force_android_api_server_toolsets, resolved_android_api_server_toolsets
    ...
    def _make_run_event_callback(self, run_id: str, loop: "asyncio.AbstractEventLoop"):
        ...
        def _callback(event_type: str, tool_name: str = None, preview: str = None, args=None, **kwargs):
            ts = time.time()
            # Delegation attribution: the run's agent exposes its subagent
            # depth (0 = top level) and, inside a child agent, the parent
            # subagent id — enough for clients to render a delegation tree.
            agent_ref = self._active_run_agents.get(run_id)
            depth = getattr(agent_ref, "_delegate_depth", 0) or 0
            parent_task_id = getattr(agent_ref, "_parent_subagent_id", None)
            if event_type == "tool.started":
                ...
                if parent_task_id:
                    event["parent_task_id"] = parent_task_id
                _push(event)
            elif event_type == "device.progress":
                # Long device operations (facade calls) report spinner text.
                _push({
                    "event": "device.progress",
                    "run_id": run_id,
                    "timestamp": ts,
                    "operation": kwargs.get("operation") or tool_name or "",
                    "status": preview or "",
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        model_override = model_raw.strip() if isinstance(model_raw, str) and model_raw.strip() else None
        run_id, session_id = self._start_run(
            user_message=user_message,
            conversation_history=conversation_history,
            instructions=instructions,
    ...
    def _start_run(
        ...
        *,
        ...
        """Launch an agent run in the background; returns (run_id, session_id).

        Shared by POST /v1/runs and busy-mode queue chaining.
        """
        ...
        async def _run_and_close():
            try:
                ...
                # Clarify: mirror the gateway's clarify_callback so the
                # `clarify` tool surfaces a clarify.request SSE event and the
                # client resolves it via POST /v1/runs/{id}/clarify.
                def _clarify_callback_sync(question: str, choices) -> str:
                    ...
                    self._set_run_status(run_id, "waiting_for_clarify", last_event="clarify.request")
                    try:
                        q.put_nowait({
                        ...
                        pass
                    timeout = float(_cg.get_clarify_timeout())
                    response = _cg.wait_for_response(clarify_id, timeout=timeout)
                    try:
                        q.put_nowait({
                ...
                def _run_sync():
                    ...
                    try:
                        ...
                        # Forward MoA member/aggregate progress into the SSE stream.
                        def _moa_cb(payload: Dict[str, Any]) -> None:
                            event = dict(payload)
                            event.update({"run_id": run_id})
                            try:
                                loop.call_soon_threadsafe(q.put_nowait, event)
                            except Exception:
                                pass
                        ...
                        try:
                            ...
                            register_moa_notify(approval_session_key, _moa_cb)
                        except Exception:
                            register_moa_notify = None  # type: ignore[assignment]

                        def _moa_cleanup() -> None:
                            if register_moa_notify is not None:
                                try:
                                    unregister_moa_notify(approval_session_key)  # noqa: F821
                                except Exception:
                                    pass

                        moa_cleanup = _moa_cleanup
                        ...
                        try:
                            ...
                            if moa_cleanup is not None:
                                moa_cleanup()
                            # Release any blocked clarify waiters on run end.
                            try:
                                from tools import clarify_gateway as _cg_cleanup

                                _cg_cleanup.clear_session(approval_session_key)
                            except Exception:
                                pass
                ...
                if isinstance(result, dict) and result.get("failed"):
                    ...
                    # Busy-mode queue: chain the next queued input as a fresh
                    # run on the same session once this one completes cleanly.
                    try:
                        await self._drain_session_queue(
                        ...
                        logger.warning("[api_server] queue drain failed after run %s: %s", run_id, drain_err)
            except asyncio.CancelledError:
                self._set_run_status(
                    run_id,
                    "cancelled",
    ...
    async def _drain_session_queue(
        ...
        *,
        ...
        """Launch queued inputs as fresh runs; returns the number chained."""
        drained = 0
        while True:
            queue = self._run_input_queues.get(approval_session_key)
            next_item = queue.popleft() if queue else None
            if not next_item:
                break
            drained += 1
            self._start_run(
                user_message=str(next_item.get("message", "")),
                conversation_history=[],
                instructions=instructions,
                session_id=session_id,
                gateway_session_key=gateway_session_key,
        ...
        if drained:
            self._run_input_queues.pop(approval_session_key, None)
        return drained

    async def _handle_run_clarify(self, request: "web.Request") -> "web.Response":
        """POST /v1/runs/{run_id}/clarify — resolve a pending run clarify prompt."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        if response is None and "text" in body:
            response = body.get("text")
        if not clarify_id:
            return web.json_response(
                _openai_error("Missing 'clarify_id'", code="invalid_clarify_request"),
                status=400,
            )

        from tools.clarify_gateway import resolve_gateway_clarify

        resolved = resolve_gateway_clarify(clarify_id, str(response) if response is not None else "")
        if not resolved:
            return web.json_response(
                _openai_error(
                    f"No pending clarify '{clarify_id}' for run {run_id}",
    ...
    async def _handle_run_steer(self, request: "web.Request") -> "web.Response":
        """POST /v1/runs/{run_id}/steer — inject a note into the running agent."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        if not message.strip():
            return web.json_response(
                _openai_error("Missing 'message'", code="invalid_steer_request"),
                status=400,
            )
        ...
        try:
            ...
            logger.warning("[api_server] steer for run %s failed: %s", run_id, exc)
        if not accepted:
            return web.json_response(
                _openai_error(f"Steer rejected for run {run_id}", code="steer_rejected"),
                status=409,
            )
    ...
    async def _handle_run_queue(self, request: "web.Request") -> "web.Response":
        """POST /v1/runs/{run_id}/queue — enqueue input for busy-mode chaining.

        The queued message starts a fresh run on the same session as soon as
        the current one completes successfully.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        if run_id not in self._active_run_tasks or run_id not in self._active_run_agents:
            return web.json_response(
                _openai_error(
                    f"Run {run_id} is not active; POST /v1/runs to start a new turn instead.",
                    code="run_not_active",
                ),
                status=409,
            )
        ...
        if not message.strip():
            return web.json_response(
                _openai_error("Missing 'message'", code="invalid_queue_request"),
                status=400,
            )

        approval_session_key = self._run_approval_sessions.get(run_id) or run_id
        queue = self._run_input_queues.setdefault(approval_session_key, deque())
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app.router.add_post("/v1/runs/{run_id}/queue", self._handle_run_queue)
            self._app.router.add_post("/v1/runs/{run_id}/stop", self._handle_stop_run)
            # Optional extension routes (e.g. embedded-runtime management
            # channel). The registrar is injected via PlatformConfig.extra by
            # embedders that share this aiohttp app; core stays unaware of it.
            _extra_registrar = (self._config.extra or {}).get("extra_route_registrar")
            if callable(_extra_registrar):
                try:
                    _extra_registrar(self._app)
                except Exception as reg_exc:
                    logger.warning("[%s] extra route registrar failed: %s", self.name, reg_exc)
