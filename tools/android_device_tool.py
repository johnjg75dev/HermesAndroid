...
The ``hermes_android`` runtime package (installed alongside Hermes on the
device) provides the device-proxy implementations. On desktop checkouts the
import fails gracefully: registration is skipped so tool discovery stays
warning-free and the tools are simply unavailable.
"""
...
try:
    from hermes_android.device.proxy import (
    ...
    _DEVICE_PROXY_AVAILABLE = True
except ImportError:  # pragma: no cover - desktop environments
    _DEVICE_PROXY_AVAILABLE = False
...
def check_requirements() -> bool:
    return (
        _DEVICE_PROXY_AVAILABLE
        and bool(os.getenv("HERMES_ANDROID_BOOTSTRAP", "").strip())
    )
