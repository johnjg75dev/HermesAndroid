...
class LogcatFacadeImpl(private val context: Context) : DeviceFacade {

    override suspend fun logcatRead(
        filterSpec: String,
        maxLines: Int,
        runId: String?,
    ): Result<String> {
        return try {
            val process = Runtime.getRuntime().exec(arrayOf("logcat", "-d", "-t", maxLines.toString(), filterSpec))
            val output = process.inputStream.bufferedReader().readText()
            process.waitFor()
            Result.success(output)
        } catch (e: Exception) {
            Result.failure(HermesError(HermesErrorCode.DEVICE_UNAVAILABLE, "Logcat read failed: ${e.message}", cause = e))
        }
    }
