...
class NotificationsFacadeImpl(private val context: Context) : DeviceFacade {

    override suspend fun notificationsList(
        limit: Int,
        runId: String?,
    ): Result<List<NotificationInfo>> {
        return try {
            val listener = NotificationListenerService::class.java
            // This requires NotificationListenerService to be enabled
            // For now, return empty list as placeholder
            Result.success(emptyList())
        } catch (e: Exception) {
            Result.failure(HermesError(HermesErrorCode.DEVICE_UNAVAILABLE, "Notifications list failed: ${e.message}", cause = e))
        }
    }
