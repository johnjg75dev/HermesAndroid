...
class ShareFacadeImpl(private val context: Context) : DeviceFacade {

    override suspend fun shareSend(
        content: String,
        mimeType: String,
        runId: String?,
    ): Result<Unit> {
        return try {
            ...
            Result.failure(HermesError(HermesErrorCode.DEVICE_UNAVAILABLE, "Share failed: ${e.message}", cause = e))
        }
    }
