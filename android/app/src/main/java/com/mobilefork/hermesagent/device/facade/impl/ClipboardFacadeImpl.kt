...
class ClipboardFacadeImpl(private val context: Context) : DeviceFacade {
    ...
    override suspend fun clipboardGet(runId: String?): Result<String> {
        return try {
            ...
            Result.failure(HermesError(HermesErrorCode.DEVICE_UNAVAILABLE, "Clipboard get failed: ${e.message}", cause = e))
        }
    }

    override suspend fun clipboardSet(
        content: String,
        runId: String?,
    ): Result<Unit> {
        return try {
            val clip = ClipData.newPlainText("hermes_clipboard", content)
            clipboardManager.setPrimaryClip(clip)
            Result.success(Unit)
        } catch (e: Exception) {
            Result.failure(HermesError(HermesErrorCode.DEVICE_UNAVAILABLE, "Clipboard set failed: ${e.message}", cause = e))
        }
    }
