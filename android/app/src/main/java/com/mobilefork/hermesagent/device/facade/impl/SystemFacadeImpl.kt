...
class SystemFacadeImpl(private val context: Context) : DeviceFacade {
    ...
    override suspend fun systemInfo(runId: String?): Result<SystemInfo> {
        return try {
            ...
            Result.failure(HermesError(HermesErrorCode.DEVICE_UNAVAILABLE, "System info failed: ${e.message}", cause = e))
        }
    }

    override suspend fun systemAction(
        action: String,
        args: Map<String, Any>?,
        runId: String?,
    ): Result<Any> {
        return try {
            ...
            val result = python.getModule("hermes_android.device.proxy")
                .callAttr("system_action", action, args?.toString() ?: "{}")
                .toString()
            Result.success(result)
        } catch (e: Exception) {
            Result.failure(HermesError(HermesErrorCode.DEVICE_UNAVAILABLE, "System action failed: ${e.message}", cause = e))
        }
    }
