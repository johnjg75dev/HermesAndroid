...
class AppsFacadeImpl(private val context: Context) : DeviceFacade {
    ...
    override suspend fun appsList(runId: String?): Result<List<AppInfo>> {
        return try {
            ...
            Result.failure(HermesError(HermesErrorCode.DEVICE_UNAVAILABLE, "Apps list failed: ${e.message}", cause = e))
        }
    }

    override suspend fun appsLaunch(
        packageName: String,
        runId: String?,
    ): Result<Unit> {
        return try {
            ...
            if (intent == null) {
                return Result.failure(HermesError(HermesErrorCode.DEVICE_UNAVAILABLE, "No launch intent for $packageName"))
            }
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            context.startActivity(intent)
            Result.success(Unit)
        } catch (e: Exception) {
            Result.failure(HermesError(HermesErrorCode.DEVICE_UNAVAILABLE, "App launch failed: ${e.message}", cause = e))
        }
    }
