...
# The hermes-android runtime package is a standalone src-layout package under
# hermes_android/ (installed as its own wheel on devices). Make it importable
# for the desktop test suite so Android-adjacent code paths (gateway api_server
# defaults, tools/android_device_tool, bridge tests) resolve exactly like they
# do on-device where both wheels are installed.
_HERMES_ANDROID_SRC = PROJECT_ROOT / "hermes_android" / "src"
if _HERMES_ANDROID_SRC.is_dir() and str(_HERMES_ANDROID_SRC) not in sys.path:
    sys.path.insert(0, str(_HERMES_ANDROID_SRC))
