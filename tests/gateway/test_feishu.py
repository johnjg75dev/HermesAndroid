...
class _EnvironmentPatch(ContextDecorator):
    def __init__(self, values, clear):
        self.values = values
        self.clear = clear
        self.patcher = None
    ...
    def __enter__(self):
        ...
        if self.clear:
            isolated = {name: os.environ[name] for name in _ENV_PASSTHROUGH if name in os.environ}
            isolated.setdefault("PYTHONIOENCODING", "utf-8")
            isolated.setdefault("PYTHONUTF8", "1")
            isolated.update(values)
            values = isolated
        self.patcher = patch.dict(os.environ, values, clear=self.clear)
        return self.patcher.__enter__()

    def __exit__(self, *exc_info):
        return self.patcher.__exit__(*exc_info)


def _patch_env(values, clear=False):
    """Patch env lazily without deleting process-critical Windows keys."""
    return _EnvironmentPatch(values, clear)
...
class TestAdapterBehavior(unittest.TestCase):
    ...
    @_patch_env( {"FEISHU_GROUP_POLICY": "open"}, clear=True)
    ...
    def test_download_remote_document_reads_response_before_httpx_client_closes(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with _patch_env( {"HERMES_HOME": tmp}, clear=False):
                adapter = FeishuAdapter(PlatformConfig())
    ...
    def test_dedup_state_persists_across_adapter_restart(self):
        ...
        with tempfile.TemporaryDirectory() as temp_home:
            with _patch_env( {"HERMES_HOME": temp_home}, clear=False):
...
class TestHydrateBotIdentity(unittest.TestCase):
    ...
    @_patch_env( {"FEISHU_BOT_OPEN_ID": "ou_env"}, clear=True)
...
class TestWebhookSecurity(unittest.TestCase):
    ...
    def _make_adapter(self, encrypt_key: str = "") -> "FeishuAdapter":
        ...
        with _patch_env( {"FEISHU_APP_ID": "cli", "FEISHU_APP_SECRET": "sec", "FEISHU_ENCRYPT_KEY": encrypt_key}, clear=True):
            return FeishuAdapter(PlatformConfig())
    ...
    @_patch_env( {"FEISHU_ENCRYPT_KEY": "secret"}, clear=True)
...
class TestDedupTTL(unittest.TestCase):
    ...
    def test_load_tolerates_malformed_timestamp_values(self):
        ...
        with tempfile.TemporaryDirectory() as temp_home:
            with _patch_env( {"HERMES_HOME": temp_home}, clear=True):
                adapter = FeishuAdapter(PlatformConfig())
...
class TestGroupMentionAtAll(unittest.TestCase):
    ...
    @_patch_env( {"FEISHU_GROUP_POLICY": "allowlist", "FEISHU_ALLOWED_USERS": "ou_allowed"}, clear=True)
...
class TestProcessingReactions(unittest.TestCase):
    ...
    # ------------------------------------------------------------- env toggle
    @_patch_env( {"FEISHU_REACTIONS": "false"}, clear=True)
